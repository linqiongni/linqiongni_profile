  ...
  <div id="tab-about-content" className="lux-page py-5 sm:py-10">
    <section className="grid gap-12 border-b border-[#D8D0C2] pb-16 lg:grid-cols-[0.78fr_1.22fr] lg:gap-20">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .08 }} className="flex flex-col justify-center">
        ...
        <h2 className="lux-title mt-5">在法律判断之外，<br /><span className="text-[#987B47]">理解商业如何运转。</span></h2>
        <p className="mt-8 max-w-3xl text-base leading-8 text-[#68655F]">{PERSONAL_INFO.bio.introParagraph1}</p>
        <blockquote className="mt-8 border-l border-[#B89F6B] pl-6 font-serif text-lg leading-8 text-[#343C50]">“我不只判断什么不能做，更关注业务怎样可以稳妥地做成。”</blockquote>
        <div className="mt-9 grid gap-x-8 gap-y-4 sm:grid-cols-2">
          {PROFILE_FACTS.filter(f => f.label !== '政治面貌').map(f => <div key={f.label} className="border-t border-[#D8D0C2] pt-3"><p className="text-[10px] uppercase tracking-[.16em] text-[#9A948A]">{f.label}</p><p className="mt-1.5 text-sm leading-6">{f.value}</p></div>)}
        </div>
        <button onClick={onExploreCases} className="mt-10 inline-flex w-fit items-center gap-2 border-b border-[#8D754B] pb-1 text-sm text-[#705E3D] transition-colors hover:text-[#B89F6B]">查看代表项目 <ArrowUpRight size={15} /></button>
      </motion.div>
    </section>

    <section className="py-16">
      <div className="grid gap-10 lg:grid-cols-[.7fr_1.3fr]">
        <div><p className="lux-kicker">Working Method</p><h3 className="mt-4 font-serif text-3xl font-normal">我的工作方法</h3><p className="mt-4 max-w-sm text-sm leading-7 text-[#77736B]">专业不是复杂术语的堆叠，而是把复杂问题处理得清楚、稳妥、可执行。</p></div>
        <div className="grid gap-px bg-[#D8D0C2] md:grid-cols-3">{PRINCIPLES.map(({icon: Icon,title,text},i)=><article key={title} className="bg-[#FDFCF9] p-7"><div className="flex items-center justify-between"><Icon size={20} strokeWidth={1.4} className="text-[#9B7B42]"/><span className="font-serif text-xs text-[#AAA296]">0{i+1}</span></div><h4 className="mt-8 text-base font-medium">{title}</h4><p className="mt-3 text-sm leading-7 text-[#77736B]">{text}</p></article>)}</div>
      </div>
    </section>

    <section className="border-t border-[#D8D0C2] pt-14">
