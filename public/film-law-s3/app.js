...
(function(){
  ...
  function renderHome(){
    var cards = EPISODES.map(function(e){
      ...
      return '<a class="ep-card" href="#/ep/'+e.id+'">' +
        '<div class="ep-no">SEASON 3 · EPISODE '+e.ep+'</div>' +
    ...
    document.getElementById('app').innerHTML =
      ...
      '<p><span class="pill">Season 3</span>用《傲骨贤妻》第三季（1–22 集）系统学习英美法律英语。每集含：真实场景台词短句引用（法律术语高亮可点击）、术语卡（英文释义 + 中文对应）、中文概览、英美制度对照。</p>' +
  ...
  function renderEpisode(id, focus){
    ...
    document.getElementById('app').innerHTML =
      '<div class="crumbs"><a href="#/">课程地图</a> › S3E'+e.ep+' · '+escapeHtml(e.title)+'</div>' +
      '<div class="ep-head"><h1>'+escapeHtml(e.title)+'</h1><div class="ep-zh">S3E'+e.ep+' · '+escapeHtml(e.zhTitle)+'</div>' +
      ...
      '<div class="section"><h2><span class="en">Scene dialogue</span> · 场景台词</h2>' +
        '<p class="sec-sub">精选自本集真实台词，高亮术语可点击跳转。Source: The Good Wife S3E'+e.ep+'.</p>' +
        scenes +
  ...
  function renderAbout(){
    document.getElementById('app').innerHTML =
      ...
      '<div class="warn">⚠ 本站点为<b>个人学习用途</b>。剧中剧本与字幕受版权保护，本站<b>不提供整集字幕或剧本原文</b>；仅<b>引用极少量短句台词</b>用于语言教学与法律评论，并逐处标注出处（The Good Wife S3E__）。术语释义、中文概览、英美制度对照均为<b>原创学习笔记</b>。如权利人认为引用不当，请联系删除。</div>' +
