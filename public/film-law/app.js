 ...
 * Depends on: transcripts.js (window.TRANSCRIPTS), data.js (window.TERMS,
...
(function(){
  ...
  var TRANSCRIPTS = window.TRANSCRIPTS || {};
  ...
  function renderHome(){
    var cards = EPISODES.map(function(e){
      ...
      return '<a class="ep-card" href="#/ep/'+e.id+'">' +
        '<div class="ep-no">SEASON 1 · EPISODE '+e.ep+'</div>' +
        ...
        '<div class="ep-meta">'+(e.terms?e.terms.length:0)+' legal terms · '+(e.scenes?e.scenes.length:0)+' scene lines · 点击查看完整字幕</div>' +
      '</a>';
    }).join('');
    document.getElementById('app').innerHTML =
      '<div class="hero"><h1>The Good Wife · Legal English Study</h1>' +
      '<p><span class="pill">Season 1</span>用《傲骨贤战》第一季（1–8 集）系统学习英美法律英语。每集含：真实场景台词（法律术语高亮可点击）、术语卡（英文释义 + 中文对应）、完整英文字幕、英美制度对照。</p>' +
  ...
  function renderEpisode(id, focus){
    ...
    var tr = TRANSCRIPTS[id] || [];
    var trHtml = tr.length ? tr.map(function(l){
      return '<div class="tr-line"><div class="sp">'+escapeHtml(l.sp)+'</div>' +
        '<div class="tx">'+(hi?hi(l.t):escapeHtml(l.t))+'</div></div>';
    }).join('') : '<div class="tr-meta">（本集字幕暂未抓取）</div>';

    document.getElementById('app').innerHTML =
      '<div class="crumbs"><a href="#/">课程地图</a> › S1E'+e.ep+' · '+escapeHtml(e.title)+'</div>' +
      ...
      '<div class="section"><h2><span class="en">Scene dialogue</span> · 场景台词</h2>' +
        '<p class="sec-sub">精选自本集真实台词，高亮术语可点击跳转。Source: The Good Wife S1E'+e.ep+'.</p>' +
        scenes +
      ...
      '<div class="section"><h2><span class="en">Legal terms</span> · 术语卡</h2>' +
        '<p class="sec-sub">英文释义 + 中文对应；点击场景/字幕中的高亮词可回到此处。</p>' +
        '<div class="term-grid">'+cards+'</div></div>' +

      '<div class="section"><h2><span class="en">Anglo-American contrast</span> · 英美制度对照</h2>' +
        uk + '</div>' +

      '<div class="section"><h2><span class="en">Full transcript</span> · 完整英文字幕</h2>' +
        '<p class="sec-sub">'+escapeHtml(SITE_NOTE)+'</p>' +
        '<button class="tr-toggle" id="trToggle">▸ 展开本集字幕（'+tr.length+' 行）</button>' +
        '<div class="tr-wrap" id="trWrap"><div class="tr-meta">英文原文字幕 · 术语同样高亮可点击。仅供个人学习（fair use）。</div>' +
        '<div class="tr-box">'+trHtml+'</div></div></div>';

    var tog = document.getElementById('trToggle');
    var wrap = document.getElementById('trWrap');
    tog.addEventListener('click', function(){
      var open = wrap.classList.toggle('open');
      tog.textContent = (open?'▾ 收起字幕':'▸ 展开本集字幕（'+tr.length+' 行）');
    });
  ...
  function renderAbout(){
    document.getElementById('app').innerHTML =
      ...
      '<li><b>Full transcript</b>：展开本集真实英文字幕，通读培养语感；里面的术语同样可点击。</li>' +
      '</ul>' +
      ...
      '<div class="warn">⚠ 本站点为<b>个人学习用途</b>。剧中剧本／字幕受版权保护，此处为<b>个人语言学习之 fair-use 汇编</b>，来源已在各字幕区块注明（springfieldspringfield.co.uk 等公开字幕源）。<b>整集字幕仅作学习展示并标注出处，非商业性传播</b>；如作商业用途须另获版权方授权。</div>' +
      '<h2><span class="en">Extend it</span> · 如何扩展</h2>' +
      '<p>所有内容都在 <span class="kbd">data.js</span> 与 <span class="kbd">transcripts.js</span>。往 <span class="kbd">EPISODES</span> 数组追加一个剧集对象即可新增一集；新术语加到 <span class="kbd">TERMS</span> 并写入该剧集的 <span class="kbd">terms</span> 列表。无需构建步骤，刷新即生效。</p>' +
      '</div>';
    window.scrollTo(0,0);
  }
